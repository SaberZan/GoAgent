...
GoAgent 支持直接登录智子云并连接远程 KataGo 算力。它和普通 iKataGo world 账号不是同一条登录路径：GoAgent 会调用智子云登录接口获取 token，再通过智子云 Socket.IO 通道启动远端 KataGo GTP，用 `kata-analyze` 读取候选点、胜率、目差和 PV。
...
3. Token 通常不需要手动填写。只有你已经从其它方式拿到了 token，才使用“高级：Token”。
4. `zz-ikatago 路径` 是兼容旧连接器的高级选项，普通用户可以不填。
5. 附加参数一般留空；只有你明确知道智子云 Socket.IO 连接参数时再填写。
...
- `智子云未配置完整`：还没有在 GoAgent 中登录智子云，请输入账号密码登录。
