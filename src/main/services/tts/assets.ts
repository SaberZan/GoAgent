...
export function inspectKokoroBundledAssets(settings: AppSettings): TtsAssetStatus {
  ...
  const modelPath = manifest ? join(rootPath, manifest.modelFile) : join(rootPath, 'onnx', 'model_quantized.onnx')
  const runtimeModelPath = manifest ? join(rootPath, manifest.runtimeModelFile ?? manifest.modelFile) : join(rootPath, 'onnx', 'model_quantized.onnx')
