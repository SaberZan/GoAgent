...
test('SGF scoring helper normalizes Fox komi and result notation before teaching', () => {
  ...
  assert.match(source, /export function normalizeSgfKomiForAnalysis/)
  assert.match(source, /function zeroKomiLooksSuspicious/)
  assert.match(source, /context\.source === 'fox'/)
  assert.match(source, /standardKomiForRules/)
  assert.match(source, /Fox SGF 原始 KM/)
...
test('Teacher tools expose normalized komi and resultSummary instead of raw Fox-only numbers', () => {
  ...
  assert.match(teacherAgent, /function komiSummaryForRecord/)
  assert.match(teacherAgent, /komi: normalizedKomi\.normalized/)
  assert.match(teacherAgent, /rules: record\.rules/)
